...
\subsection*{Recommended $\lambda$ (grouped 70/15/15 sweeps)}

\paragraph{CGCNN} (300-epoch follow-up):
\begin{itemize}
...
\paragraph{Graph Transformer} (1000-epoch $\lambda$ sweep, Mar.~2026):
\begin{itemize}
  \item \textbf{Point:} $\lambda=0.02$ (was $0.01$ at delivery). Test $R_{\mathrm{tot}}$ $17.1\%$ vs.\ $27.8\%$ at $\lambda=0.01$.
  \item \textbf{Planar:} $\lambda=0.01$ (was $0.005$). Test $R_{\mathrm{tot}}$ $41.9\%$ vs.\ $58.8\%$ at $\lambda=0.005$.
\end{itemize}
...
\begin{enumerate}
  \item Deploy global-loss v2 with model-specific $\lambda$:
  ...
  \item Expect planar CGCNN trade-off: ${\sim}0.002$~eV/atom worse MAE for ${\sim}10$~pp better $R_{\mathrm{tot}}$.
  \item Do not use legacy v1 loss or $\lambda=0.005$ on point CGCNN with $R_{\mathrm{tot}}$ checkpointing.
\end{enumerate}
...
\begin{tabular}{@{}ll@{}}
  ...
  Parameter & Value \\
  \midrule
  $\lambda$ (point, CGCNN)  & $0.01$ \\
  $\lambda$ (planar, CGCNN) & $0.005$ \\
  $\lambda$ (point, Transformer)  & $0.02$ \\
  $\lambda$ (planar, Transformer) & $0.01$ \\
