"""Analyse and plot results from the GatedConv hyperparameter sweep (v3).

Swept: hidden_dim, num_layers, dropout, lr, weight_decay, batch_size.
Fixed: activation=silu, batch_norm=False.
"""
...
def _load(path: str):
    with open(path, "r", encoding="utf-8") as f:
        payload = json.load(f)
    results = [r for r in payload["results"] if "mean_val_mae" in r]
    return results, payload


def main() -> None:
    parser = argparse.ArgumentParser(description="Plot GatedConv sweep results.")
    parser.add_argument("--input", type=str, default="sweep_gated_cycle34_v3.json")
    parser.add_argument("--output", type=str, default="sweep_gated_cycle34_v3_plot.png")
    ...
    print(f"Loaded {len(results)} valid runs  |  Fixed: {fixed_str}\n")
    ...
    ax = axes[0][1]
    jitter = np.random.default_rng(0).uniform(-0.15, 0.15, len(num_layers))
    sc = ax.scatter(num_layers + jitter, test_maes, c=hidden_dims, cmap="viridis",
                    s=15, alpha=0.6, edgecolors="none")
    fig.colorbar(sc, ax=ax, label="hidden_dim")
    ...
    ax.barh(y_pos, [r["mean_test_mae"] for r in top10], color="C1", alpha=0.8)
    ax.set_yticks(y_pos)
    ...
    footer = (
        f"Best (by val MAE): "
        f"hd={bc['hidden_dim']}  nl={bc['num_layers']}  dr={bc['dropout']}  "
        f"lr={bc['lr']}  wd={bc['weight_decay']}  bs={bc['batch_size']}  "
        f"(fixed: {fixed_str})\n"
