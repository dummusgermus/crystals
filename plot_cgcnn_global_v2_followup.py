"""Plot global v2 follow-up curves and baseline-vs-lambda summary bars."""
...
def _lambda_runs(runs: Dict[str, Dict], domain: str) -> List[Tuple[float, str, Dict]]:
    ...
    prefix = f"{domain}_global_v2_lambda_"
...
def _plot_epoch_curves(payload: dict, output_prefix: str) -> None:
    ...
    fig.suptitle(
        "Global v2 follow-up (checkpoint: baseline=MAE, global=R_tot)",
        y=1.03,
        fontsize=12,
    )
    fig.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument("--input", default="cgcnn_global_v2_followup_curves.json")
    parser.add_argument(
        "--output-prefix",
        default="cgcnn_global_v2_followup",
    )
