...
def grouped_train_val_indices(
    ...
    n_samples = len(dataset)
    target_train = max(1, int(round((1.0 - val_fraction) * n_samples)))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Single training run for node PE prediction. "
            "By default this trains the base GNN with the best hyperparameters found in the sweeps."
        )
    )
