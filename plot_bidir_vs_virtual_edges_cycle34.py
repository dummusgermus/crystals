"""Plot test MAE curves and per-epoch runtime from bidir_vs_virtual_edges_cycle34.json."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot curves from train_bidir_vs_virtual_edges_cycle34.py output JSON."
    )
    parser.add_argument("--input", type=str, default="bidir_vs_virtual_edges_cycle34.json")
    parser.add_argument("--output", type=str, default="bidir_vs_virtual_edges_cycle34_plot.png")
    parser.add_argument(
        "--curve",
        type=str,
        default="test",
    ...
    labels = {
        "bidirectional": "Bidirectional (cycle34)",
        "bidirectional_virtual_edges": "Bidirectional + virtual edges",
    }
    colors = {"bidirectional": "C0", "bidirectional_virtual_edges": "C1"}
    order = ["bidirectional", "bidirectional_virtual_edges"]
    ...
    def summary_lines() -> List[str]:
        ...
        for key in order:
            ...
            lines.append(
                f"{labels[key]:38s}  test {metric} = {final_test:.4f}  |  "
    ...
    if args.curve == "all":
        ...
        fig.suptitle("Bidirectional vs virtual edges (cycle34)", fontsize=13)
