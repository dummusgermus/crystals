"""Compare bidirectional EdgeFNNConv (base) vs CGCNN-style gated convolution.
...
from gnn_models import build_gated_model_from_dataset, build_model_from_dataset
...
DEFAULT_OUTPUT = "bidir_vs_gated_cycle34.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare bidirectional base vs CGCNN gated conv on cycle34."
    )
    ...
    # --- Model 1: bidirectional EdgeFNNConv (base) ---
    print("\n" + "=" * 60)
    print("  Model: bidirectional EdgeFNNConv (base)")
    print("=" * 60)
    set_seed(args.seed)
    base_model = build_model_from_dataset(dataset, bidirectional=True, **model_kwargs)
    curves_base = _train_one(name="bidir-base", model=base_model, **common_train_kwargs)

    # --- Model 2: bidirectional GatedConv (CGCNN Eq. 5) ---
    print("\n" + "=" * 60)
    print("  Model: bidirectional GatedConv (CGCNN Eq. 5)")
    print("=" * 60)
    set_seed(args.seed)
    gated_model = build_gated_model_from_dataset(dataset, bidirectional=True, **model_kwargs)
    curves_gated = _train_one(name="bidir-gated", model=gated_model, **common_train_kwargs)
    ...
    print("  Summary")
    print("=" * 60)
    print(f"  EdgeFNNConv (base)  -> test MAE: {_final(curves_base):.4f}  "
          f"({curves_base['total_train_time']:.0f}s, "
          f"{curves_base['num_parameters']:,} params)")
    print(f"  GatedConv (CGCNN)   -> test MAE: {_final(curves_gated):.4f}  "
    ...
    output = {
        ...
        "models": {
            "bidirectional_base": curves_base,
            "bidirectional_gated": curves_gated,
        },
        "final_test_mae": {
            "bidirectional_base": _final(curves_base),
            "bidirectional_gated": _final(curves_gated),
        },
    }
