...
def _max_energy_change_for_pair(
    ...
    for idx, pid in enumerate(particle_ids):
        ...
        if use_absolute:
            delta = abs(delta)
        if max_change is None or delta > max_change:
            max_change = float(delta)

    return max_change, missing_relaxed


def collect_max_energy_changes(
    simulations_dir: str,
    use_absolute: bool,
    ...
    for folder_path in _iter_valid_simulation_folders(simulations_dir):
        ...
        for filename, parsed in data_files:
            ...
            max_change, missing_relaxed = _max_energy_change_for_pair(
                dump_path,
                relaxed_dump_path,
    ...
    return np.asarray(max_changes, dtype=float), stats
...
def plot_histogram(
    ...
    if max_changes.size == 0:
        raise ValueError("No max-change values collected; nothing to plot.")
    ...
    plt.xlabel(f"{label} max potential energy change per graph (eV)")
    plt.ylabel("Graph count")
    plt.title(f"Max per-graph energy changes across {max_changes.size} graphs")
    plt.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Histogram of the maximum per-atom energy change per graph."
    )
    ...
    parser.add_argument(
        "--output",
        default=os.path.join(root_dir, "energy_change_max_histogram.png"),
    ...
    max_changes, stats = collect_max_energy_changes(
    ...
    print(
        "Histogram saved to "
        f"{args.output} using {max_changes.size} graphs out of "
        f"{stats['pairs_used']} pairs."
    )
    if stats["pairs_without_hits"] > 0:
        print(f"Pairs with no atoms: {stats['pairs_without_hits']}")
